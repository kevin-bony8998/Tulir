...
export const DownloadsPage:React.FC<DetailsPageProps> = ({pageClassName}) => {
          ...
          DownloadsMock?.pageContent?.map((sectionDetails, index: number) => (
            <div className="section-details-wrapper" key={index}>
              ...
              <div className="links-wrapper">
                {
                  sectionDetails?.sectionContent?.map((individualSectionDetails, index: number) => (
                    <div className="individual-link" key={index}>
                      {
                        ...
                        (
                          <div className="multiple-language-link">
                            ...
                            (
                            {
                              individualSectionDetails?.languages?.map((languageDetails, index: number) => (
                                <a href={`${languageDetails?.link}`} className="language-link downloads-link" key={index} target="_blank">
                                  {' ' + languageDetails?.language + (index === individualSectionDetails?.languages?.length - 1 ? ' ' : ' / ')} 
                                </a>
                              ))
                            }
                            )
                        ...
                        (
                          <div className="single-language-link">
                            <a href={`${individualSectionDetails?.generalLink}`} className="general-link-hyperlink downloads-link" target="_blank">
                              {individualSectionDetails?.linkTitle}
                            </a>
                          </div>
                        )
